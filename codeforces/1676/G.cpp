...
bool yes;
// ll ara[maxn];
vector<ll> adj_list[5000];

pair<ll, ll> dfs(ll dad, ll son) {
    ll whiteman = 0;
    ll blackman = 0;

    if (str[son] == 'W') whiteman = 1;
    else blackman = 1;

    for (ll grandson : adj_list[son]) {
        if (grandson == dad) continue;
        pll ret = dfs(son, grandson);
        whiteman += ret.first;
        blackman += ret.second;
    }

    if (whiteman == blackman) {
        ans++;
        db printf("> dad=%lld son=%lld w=%lld b=%lld char=%c\n", dad, son, whiteman, blackman, str[son]);
    }

    return {whiteman, blackman};
}

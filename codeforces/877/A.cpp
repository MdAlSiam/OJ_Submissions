...
void solve() {
    ...
    string names_to_find[5] = {"Danil", "Olya", "Slava", "Ann", "Nikita"};
    ...
    For (tt, 0, 5) {
        string a_name = names_to_find[tt];

        For (i, 0, str.length()) {
            if (i+a_name.length() <= str.length()) {
                if (str.substr(i, a_name.length()) == a_name) {
                    // cout << "Got " << a_name << endl;
                    times++;
                }
            }
        }
